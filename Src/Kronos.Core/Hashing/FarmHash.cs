...
namespace Kronos.Core.Hashing
{
    ...
    /// Copyrights: Nick Babcock, "nickbabcock"
    ...
    public static class Farmhash
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static uint128 Uint128(ulong lo, ulong hi) => new uint128(lo, hi);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe uint128 WeakHashLen32WithSeeds(
        ...
        private static unsafe ulong Hash64_uo(byte* s, uint len)
        {
            ...
            // v and w used to be uint128(seed0, seed1), uint128(0, 0), but
            ...
            uint128 v = WeakHashLen32WithSeeds(s, v_second * mul, x + w_first);
            uint128 w = WeakHashLen32WithSeeds(s + 32, z + w_second, y + Fetch64(s + 16));
        ...
        private static unsafe ulong Hash64_na(byte* s, uint len)
        {
            ...
            uint128 v = Uint128(0, 0);
            uint128 w = Uint128(0, 0);
            x = x * k2 + Fetch64(s);
